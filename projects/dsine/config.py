...
def get_args(test=False):
    ...
    parser.add_argument(
        "--experiment_dir", "-ed", type=str, default=None, help="override the base EXPERIMENT_DIR for outputs"
    )
    # ↓↓↓↓
    ...
    # NOTE: update args – allow override via --experiment_dir
    base_root = args.experiment_dir if args.experiment_dir else EXPERIMENT_DIR
    args.exp_root = os.path.join(base_root, "dsine")
    args.load_normal = True
    args.load_intrins = True
    # ↑↑↑↑
    ...
    if not test and not args.overwrite_models and len(glob.glob(os.path.join(args.output_dir, "models", "*.pt"))) > 0:
